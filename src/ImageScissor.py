...
class ImageScissor:
    ...
    def cut_image(self, image, rect, anchor):
        """
        ...
        mask = self._create_mask(img_array, rect)

        # Apply mask to image, keeping only foreground
        img_array_with_alpha[:, :, 3] = mask * 255  # Modify the alpha channel (4th channel) for transparency
    ...
    def _create_mask(self, img_array, rect):
